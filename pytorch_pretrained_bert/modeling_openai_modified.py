...
class OpenAIGPTModel(OpenAIGPTPreTrainedModel):
    ...
    def forward(self, persona_ids, history_ids, reply_ids, position_ids=None, history_token_type_ids=None, token_info_ids=None):
        ...
        if position_ids is None:
            ...
            history_position_ids = torch.arange(history_ids.size(-1), dtype=torch.long, device=history_ids.device)
            history_position_ids = history_position_ids.unsqueeze(0).expand_as(history_ids)

            reply_position_ids = torch.arange(reply_ids.size(-1), dtype=torch.long, device=reply_ids.device)
            reply_position_ids = reply_position_ids.unsqueeze(0).expand_as(reply_ids)


        persona_ids_shape = persona_ids.size()
        persona_ids = persona_ids.view(-1, persona_ids.size(-1))
        persona_position_ids = persona_position_ids.view(-1, persona_position_ids.size(-1))

        history_ids_shape = history_ids.size()
        history_ids = history_ids.view(-1, history_ids.size(-1))
        history_position_ids = history_position_ids.view(-1, history_position_ids.size(-1))

        reply_ids_shape = reply_ids.size()
        reply_ids = reply_ids.view(-1, reply_ids.size(-1))
        reply_position_ids = reply_position_ids.view(-1, reply_position_ids.size(-1))


        persona_embeds = self.tokens_embed(persona_ids)
        history_embeds = self.tokens_embed(history_ids)
        reply_embeds = self.tokens_embed(reply_ids)


        persona_position_embeds = self.positions_embed(persona_position_ids)
        history_position_embeds = self.positions_embed(history_position_ids)
        reply_position_embeds = self.positions_embed(reply_position_ids)




        if history_token_type_ids is not None:
            history_token_type_ids = history_token_type_ids.view(-1, history_token_type_ids.size(-1))
            history_token_type_embeds = self.tokens_embed(history_token_type_ids)
        else:
            history_token_type_embeds = 0
        ...
        reply_hidden_states = reply_embeds + reply_position_embeds
        history_hidden_states = history_embeds + history_position_embeds + history_token_type_embeds
        persona_hidden_states = persona_embeds + persona_position_embeds


        reply_hidden_states = self.drop(reply_hidden_states)
        history_hidden_states = self.drop(history_hidden_states)
...
class OpenAIGPTDoubleHeadsModel(OpenAIGPTPreTrainedModel):
    ...
    def forward(self, persona_ids, history_ids, reply_ids, mc_token_ids,
                lm_labels=None, mc_labels=None, history_token_type=None, token_info_ids=None, position_ids=None):
